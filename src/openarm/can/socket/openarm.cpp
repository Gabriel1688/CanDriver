...
OpenArm::OpenArm(const std::string& can_interface)
    ...
    master_can_device_collection_ = std::make_unique<CANDeviceCollection>(*can_socket_);
    arm_ = std::make_unique<ArmComponent>(*can_socket_);
    gripper_ = std::make_unique<GripperComponent>(*can_socket_);
}

void OpenArm::init_arm_motors(const std::vector<MotorType>& motor_types,
...
void OpenArm::init_gripper_motor(MotorType motor_type, uint32_t send_can_id,
                                 uint32_t recv_can_id) {
    gripper_->init_motor_device(motor_type, send_can_id, recv_can_id);
    register_dm_device_collection(*gripper_);
}

void OpenArm::register_dm_device_collection(DMDeviceCollection& device_collection) {
...
void OpenArm::enable_all() {
    for (DMDeviceCollection* device_collection : sub_dm_device_collections_) {
...
void OpenArm::set_callback_mode_all(CallbackMode callback_mode) {
    for (DMDeviceCollection* device_collection : sub_dm_device_collections_) {
